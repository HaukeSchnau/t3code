...
const makeHarness = Effect.fn("TestThreadPagination.makeHarness")(function* (options?: {
  ...
  const snapshotLoader = ThreadSnapshotLoader.of({
    ...
    load: (_prepared, _threadId, _activityDetailMode, window, reasoningMessages) =>
      Effect.gen(function* () {
        yield* Ref.update(loaderWindows, (current) => [...current, window]);
        yield* Ref.update(loaderReasoning, (current) => [...current, reasoningMessages]);
        const response = yield* window?.beforeCursor === undefined
          ? Effect.succeed(
