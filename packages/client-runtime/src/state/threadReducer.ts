...
function compactInactiveTurnActivities(
  ...
  // Streaming deltas rarely move activity out of the hot turns. Keep both
  // references so work-log selectors skip unchanged history.
  if (demotedByTurnId.size === 0 && thread.historicalActivityGroups !== undefined) {
    return {
...
export function applyThreadDetailEvent(
  ...
  switch (event.type) {
    ...
    case "thread.created":
      return {
        ...
        thread: {
          ...
          ...(event.payload.titleMode !== undefined ? { titleMode: event.payload.titleMode } : {}),
    ...
    case "thread.message-sent": {
      ...
      // Streaming deltas recompute the same record; keep the previous reference
      ...
      // helper returns the same array when the entry is already bound.
      const checkpoints =
 ...
 * Streaming recomputes the latest turn on every delta; keeping the old
 * reference lets selectors and memos keyed on `latestTurn` skip work.
 */
 ...
 * when nothing needs rebinding, so streaming deltas for an already-bound
 * message do not allocate a new `checkpoints` reference.
 */
