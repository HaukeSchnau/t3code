...
function formatDisconnectDetail(
  ...
  return `${base} WebSocket close code ${closeDiagnostics.code ?? "unavailable"}${reason}.`;
}
    ...
    const diagnosticWebSocketConstructor: typeof webSocketConstructor = (url, protocols) => {
      ...
      socket.addEventListener(
        ...
        (event) => {
          ...
          closeDiagnostics = { code: event.code, reason };
        },
        { once: true },
      );
      return socket;
        ...
        Effect.flatMap((wasConnected) =>
          Deferred.fail(
            ...
            new ConnectionTransientErrorClass({
              reason: "transport",
              detail: `${formatDisconnectDetail(connection.label, wasConnected, closeDiagnostics)}${networkHint}`,
            }),
          ),
        ),
        Effect.asVoid,
