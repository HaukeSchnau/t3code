...
describe("resolveSelectableModel", () => {
  it("resolves exact slugs, labels, and aliases", () => {
    ...
    expect(resolveSelectableModel(ProviderDriverKind.make("codex"), "gpt-5", options)).toBe(
      "gpt-5.5",
    );
    expect(resolveSelectableModel(ProviderDriverKind.make("codex"), "gpt-5-codex", options)).toBe(
      "gpt-5.5",
    );
