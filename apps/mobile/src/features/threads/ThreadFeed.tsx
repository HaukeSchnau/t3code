...
function MarkdownInlineCode(props: {
  ...
  const link = insideLink || presentation ? null : resolveInlineCodeWebLink(props.content);
  const href = presentation?.href ?? link?.href;
  return (
    <NativeText
      className={presentation ? "font-t3-bold" : "font-mono"}
      onPress={href ? () => props.onLinkPress(href) : undefined}
      style={{
        color: presentation ? props.textColor : link ? props.linkColor : props.codeColor,
