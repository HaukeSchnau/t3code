...
it.layer(NodeServices.layer)("message context plumbing", (it) => {
  ...
  it.effect("preserves context through immediate and deferred queued delivery", () =>
    Effect.gen(function* () {
      const readModel = makeReadModel();
      const result = yield* decideOrchestrationCommand({
        command: {
          type: "thread.message.queue",
          commandId: CommandId.make("queue-context"),
          threadId: ThreadId.make("thread-1"),
          message: {
      ...
      const events = Array.isArray(result) ? result : [result];
      const immediate = events.find((event) => event.type === "thread.message-sent");
      expect(
        immediate?.type === "thread.message-sent" ? immediate.payload.context : undefined,
      ).toEqual(context);
      const queued = events.find((event) => event.type === "thread.message-queued");
      if (!queued) throw new Error("Expected queued message event");
      const queuedState = yield* projectEvent(readModel, { ...queued, sequence: 1 });
      expect(queuedState.threads[0]?.queuedMessages?.[0]?.context).toEqual(context);
      const dispatched = yield* decideOrchestrationCommand({
        ...
        readModel: queuedState,
      });
      const sent = (Array.isArray(dispatched) ? dispatched : [dispatched]).find(
