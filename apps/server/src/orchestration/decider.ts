  ...
  switch (command.type) {
    ...
    case "project.meta.update": {
      ...
      if (command.scripts !== undefined) {
        // Existing script ids predate shortcut validation. Preserve those ids
        // while rejecting newly introduced commands that cannot be invoked.
      ...
      return {
        ...
        payload: {
          ...
          ...(title !== undefined
            ? {
                title,
                titleMode: command.titleMode ?? ("manual" as const),
                titleState: {
                  source:
                    command.titleMode === "automatic"
                      ? ("generated" as const)
                      : ("manual" as const),
    ...
    case "thread.message.queue": {
      ...
      const queuedMessage = {
        ...
        threadId: command.threadId,
        text: command.message.text,
        ...(command.message.context ? { context: command.message.context } : {}),
        attachments: command.message.attachments,
        ...(command.message.origin !== undefined ? { origin: command.message.origin } : {}),
    ...
    case "thread.user-input.respond": {
      ...
      if (attachments.length > 0) {
        ...
        for (const questionId of Object.keys(command.attachmentsByQuestionId ?? {})) {
          const question = payload.value.questions.find((entry) => entry.id === questionId);
    ...
    case "thread.history.prune": {
      ...
      return {
        ...
        type: "thread.history-prune-requested",
        payload: {
          threadId: command.threadId,
          messageId: command.messageId,
          createdAt: command.createdAt,
    ...
    case "thread.history.prune.complete": {
      yield* requireThread({
        readModel,
        command,
        threadId: command.threadId,
      ...
      return {
        ...
        payload: {
          threadId: command.threadId,
          messageId: command.messageId,
          pruneFromCreatedAt: command.pruneFromCreatedAt,
          prunedTurnIds: command.prunedTurnIds,
        },
      };
    }
