...
describe("normalizeDispatchCommand attachments", () => {
  ...
  it.effect("rejects understated image metadata before writing any files", () =>
    Effect.gen(function* () {
      const config = yield* ServerConfig.ServerConfig;
      ...
      expect(error.message).toContain("metadata does not match");
        ...
        turnStartCommand({
          ...
          context: {
            ...
            records: [
              {
                version: 1,
                contextId: ComposerContextId.make("local-image-1"),
                kind: "image",
              ...
              {
                version: 1,
                contextId: ComposerContextId.make("ctx-skill"),
                kind: "skill",
                label: "$review",
                name: "review",
              },
  ...
  for (const type of ["thread.turn.start", "thread.message.queue"] as const) {
    it.effect(`claims uploaded documents and context for ${type}`, () =>
      Effect.gen(function* () {
        const config = yield* ServerConfig.ServerConfig;
              ...
              records: [
                {
                  version: 1 as const,
                  kind: "file" as const,
                  contextId: ComposerContextId.make("document-1"),
                  label: "report.pdf",
        ...
        if (normalized.type !== "thread.turn.start" && normalized.type !== "thread.message.queue") {
          throw new Error("Expected a thread.turn.start command.");
        }
        ...
        expect(normalized.message.context?.records[0]).toMatchObject({
