...
/* oxlint-disable t3code/no-manual-effect-runtime-in-tests -- This integration suite owns explicit runtime lifecycle and restart coverage. */
...
type ReceiptRepositoryLayer = Layer.Layer<
  OrchestrationCommandReceiptRepository,
  never,
  SqlClient.SqlClient
>;
...
async function createPersistentOrchestrationSystem(
  ...
  return createOrchestrationSystem(databasePath, receiptRepositoryLayer);
}
...
async function prepareLegacyReceiptDatabase(
  ...
  const runtime = ManagedRuntime.make(NodeSqliteClient.layer({ filename: databasePath }));
