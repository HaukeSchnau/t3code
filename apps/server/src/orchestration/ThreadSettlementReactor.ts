...
export const make = Effect.gen(function* () {
  ...
  const start: ThreadSettlementReactor["Service"]["start"] = Effect.fn(
    ...
    // TODO: Remove the fallback when engine test doubles expose scoped subscriptions.
    const events = yield* engine.subscribeDomainEvents ?? Effect.succeed(engine.streamDomainEvents);
