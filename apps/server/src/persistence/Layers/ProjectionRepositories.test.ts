...
projectionRepositoriesLayer("Projection repositories", (it) => {
  ...
  it.effect("stores thread model options and fork workspace metadata", () =>
    Effect.gen(function* () {
      ...
      yield* threads.upsert({
        ...
        titleMode: "manual",
        workspaceId: ThreadWorkspaceId.make("workspace-options"),
        skillScope: { version: 1, appliedVersion: 1, packIds: [], state: "ready" },
        modelSelection: {
      ...
      assert.strictEqual(Option.getOrThrow(persisted).titleMode, "manual");
      assert.strictEqual(Option.getOrThrow(persisted).workspaceId, "workspace-options");
      assert.deepStrictEqual(Option.getOrThrow(persisted).skillScope, {
        version: 1,
        appliedVersion: 1,
        packIds: [],
        state: "ready",
