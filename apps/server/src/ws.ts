  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const canReplayPersistedRange = Effect.fnUntraced(function* (
        ...
        const getEventReplayStats = projectionSnapshotQuery.getEventReplayStats;
        if (getEventReplayStats === undefined) {
          return true;
        }
        const stats = yield* getEventReplayStats({
      ...
      const toDispatchCommandError = (cause: unknown, fallbackMessage: string) =>
        cause instanceof Error
          ? cause
          : new Error(cause === undefined ? fallbackMessage : String(cause));
      ...
      const commandDispatchWorkflow = makeOrchestrationCommandDispatchWorkflow({
        ...
        dispatchCommand: dispatchFromClient,
        onCommandDispatched: (command) =>
          recordClientCommandAnalytics(command).pipe(
            Effect.andThen(
              ProjectCloneTracker.discardCloneForDeletedProject(projectCloneTracker, command),
            ),
          ),
      });
            ...
            projectCloneTracker.start(input, {
              createProject: (project) =>
                Effect.gen(function* () {
                  ...
                  yield* commandDispatchWorkflow.dispatchNormalizedCommand(normalizedCommand);
                ...
                repositoryIdentityResolver.resolve(project.workspaceRoot, { refresh: true }).pipe(
                  Effect.andThen(
                    Effect.gen(function* () {
                      ...
                      yield* commandDispatchWorkflow.dispatchNormalizedCommand(command);
                    }),
                  ),
        ...
        const rpcWebSocketHttpEffect = yield* Effect.gen(function* () {
          ...
          Effect.provide(
            makeWsRpcLayer(
              ...
              Layer.provide(
                Layer.succeed(LocalAgentAwareness.LocalAgentAwareness, localAgentAwareness),
              ),
