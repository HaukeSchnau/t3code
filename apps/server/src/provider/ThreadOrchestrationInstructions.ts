...
For managed workers, use one preferred lifecycle: create or fork the workers, let them communicate directly unless the task requires independence, register one durable wait with \`t3 thread wait create\`, then end your turn. The wait survives server restarts and wakes you when the workers settle or need attention. Do not poll with \`t3 thread await\` for routine progress. Use \`t3 thread watch create\` for external command output or WebSocket events, not for thread completion.
