...
sessionErrorLayer("CodexAdapterLive session errors", (it) => {
  ...
  it.effect("prompts Codex to continue an interrupted turn", () =>
    Effect.gen(function* () {
      const adapter = yield* CodexAdapter;
      const threadId = asThreadId("sess-continuation");
      yield* adapter.startSession({
        provider: ProviderDriverKind.make("codex"),
        threadId,
        runtimeMode: "full-access",
      ...
      yield* Effect.ignore(adapter.sendTurn({ threadId, continuation: true }));
...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  ...
  it.effect("maps MCP startup status updates to canonical status events", () =>
    Effect.gen(function* () {
      const { adapter, runtime } = yield* startLifecycleRuntime();
      const firstEventFiber = yield* Stream.runHead(adapter.streamEvents).pipe(Effect.forkChild);
      ...
      NodeAssert.equal(firstEvent.value.type, "mcp.status.updated");
      if (firstEvent.value.type !== "mcp.status.updated") {
        return;
      }
      NodeAssert.deepStrictEqual(firstEvent.value.payload.status, {
        threadId: "provider-thread-1",
        name: "creative_production_mcp",
        status: "starting",
      });
...
it.effect("flushes managed native logs when the adapter layer shuts down", () =>
  Effect.gen(function* () {
    const tempDir = NodeFS.realpathSync(
      NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "t3-codex-adapter-native-log-")),
    );
