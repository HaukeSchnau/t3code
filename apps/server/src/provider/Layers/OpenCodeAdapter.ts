...
/** Keep provider runtime metadata adjacent to the fork's durable thread workflow guidance. */
export const buildOpenCodeInstructions = (model: string): string =>
  `${buildRuntimeInstructions({ harness: "OpenCode", model })}${T3_CODE_THREAD_ORCHESTRATION_INSTRUCTIONS}`;
...
export function makeOpenCodeAdapter(
  ...
  return Effect.gen(function* () {
    ...
    // event has not landed yet. The durability gate cannot sit in front of a
    // synchronous publish, so the returned effect journals the event afterwards.
    const emitUnsafe = (event: ProviderRuntimeEvent) => {
      Queue.offerUnsafe(runtimeEvents, event);
      return acceptRuntimeEvent(event).pipe(Effect.asVoid);
    };
    ...
    const closePendingOpenCodeRequests = Effect.fn("closePendingOpenCodeRequests")(function* (
      ...
      for (const request of questions) {
        ...
        context.emittedTerminalRequestIds.add(request.id);
        yield* emitUnsafe({ ...base, type: "user-input.resolved", payload: { answers: {} } });
      }
    });
            ...
            context.client.session.promptAsync(
              {
                ...
                system: buildOpenCodeInstructions(
