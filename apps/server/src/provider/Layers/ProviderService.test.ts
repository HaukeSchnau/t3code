...
multiCodex.layer("ProviderServiceLive provider instance handoff", (it) => {
  it.effect("stops the bound instance before starting its replacement", () =>
    Effect.gen(function* () {
      ...
      const cwd = fixtureCwd("shared-project");
      ...
      assert.equal(replacementInput?.cwd, cwd);
...
routing.layer("ProviderServiceLive routing", (it) => {
  ...
  it.effect("persists the rolled-back provider resume cursor before the next recovered turn", () =>
    Effect.gen(function* () {
      ...
      const cwd = fixtureCwd("project-rollback-cursor");
      ...
      const persistedAfterRollback = yield* runtimeRepository.getByThreadId({ threadId });
...
describe("agent browser access", () => {
  ...
  const startSessionWith = (
    ...
    Effect.gen(function* () {
      const issued: Array<ReadonlySet<string>> = [];
      const codex = makeFakeCodexAdapter();
      const providerAdapterLayer = Layer.succeed(
        ProviderAdapterRegistry.ProviderAdapterRegistry,
      ...
      const providerLayer = makeProviderServiceLive({
        issueMcpCredential: (request) =>
          Effect.sync(() => {
            issued.push(request.capabilities);
            return undefined;
          }),
      }).pipe(
        Layer.provide(NodeServices.layer),
        ...
        Layer.provide(
          ServerSettings.ServerSettingsService.layerTest({
            enableAgentBrowserAccess: typeof access === "boolean" ? access : access.browser,
            enableAgentDeviceAccess: typeof access === "boolean" ? false : access.device,
  ...
  it.effect("keeps PR tools when browser access is disabled", () =>
    Effect.gen(function* () {
      const issued = yield* startSessionWith(false, asThreadId("thread-browser-off"));
      assert.equal(issued.length, 1);
      assert.equal(issued[0]?.has("pull-requests"), true);
      assert.equal(issued[0]?.has("preview"), false);
    }),
  );

  it.effect("grants preview capability when browser access is enabled", () =>
    Effect.gen(function* () {
      const issued = yield* startSessionWith(true, asThreadId("thread-browser-on"));
      assert.equal(issued[0]?.has("preview"), true);
    }),
  );

  it.effect("honors project overrides in both directions", () =>
    Effect.gen(function* () {
      const denied = yield* startSessionWith(true, asThreadId("thread-project-off"), false);
      const allowed = yield* startSessionWith(false, asThreadId("thread-project-on"), true);
      assert.equal(denied[0]?.has("preview"), false);
      assert.equal(denied[0]?.has("pull-requests"), true);
      assert.equal(allowed[0]?.has("preview"), true);
    }),
  );
  ...
  it.effect("a project device override grants device access when the environment denies it", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [new Set(["device", "pull-requests"])]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("withholds only the overridden capability when the project cannot be resolved", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [new Set(["preview", "pull-requests"])]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
});
