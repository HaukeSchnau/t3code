...
const LEGACY_WORKSPACE_OPEN_SCHEME = "t3";

function resolveProtocolClientLaunchArgs(input: {
...
function registerDefaultProtocolClient(input: {
...
export const make = Effect.gen(function* () {
  ...
  return DesktopClerk.of({
    configure: Effect.gen(function* () {
      ...
      yield* registerDefaultProtocolClient({
        electronApp,
        protocol: desktopScheme,
        isDevelopment: environment.isDevelopment,
      });
      yield* registerDefaultProtocolClient({
        electronApp,
      ...
      yield* electronApp.on<[Electron.Event, string]>("open-url", (event, rawUrl) => {
        ...
        void runPromise(openWorkspace.dispatchUrl(rawUrl));
      });
      ...
      yield* electronApp.on<[Electron.Event, readonly string[]]>(
        ...
        (_event, argv) => {
          void runPromise(
            Effect.gen(function* () {
              for (const value of resolveProtocolClientLaunchArgs({ argv })) {
