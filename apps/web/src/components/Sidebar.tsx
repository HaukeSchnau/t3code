...
export const SidebarThreadRow = memo(function SidebarThreadRow(props: SidebarThreadRowProps) {
  ...
  const isPinned = useUiStateStore((state) => state.threadPinnedAtById[threadKey] !== undefined);
  ...
  const isMultilineProjectRow = props.projectLabel != null;
  ...
  const handleTogglePinClick = useCallback(
    ...
    [isPinned, setThreadPinned, threadKey],
  );
    ...
    >
      <SidebarMenuSubButton
        ...
        className={`${resolveThreadRowClassName({
          isActive,
          isSelected,
          isMultiline: isMultilineProjectRow,
        })} relative isolate`}
          ...
          {props.projectLabel ? (
            <Tooltip>
              <TooltipTrigger
                render={
                  <span
                    className="flex min-w-0 max-w-full items-center gap-1.5 text-[10px] leading-none text-muted-foreground/62"
                    data-testid={`thread-project-${thread.id}`}
                  >
                    <span
                      aria-hidden="true"
                      className="size-1.5 shrink-0 rounded-full bg-muted-foreground/38"
                    />
                    <span className="min-w-0 truncate">{props.projectLabel}</span>
                  </span>
                }
              />
              <TooltipPopup side="top" className="max-w-80 whitespace-normal leading-tight">
                {props.projectTooltip ?? props.projectLabel}
              </TooltipPopup>
            </Tooltip>
          ) : null}
        </div>
              ...
              <span className="inline-flex items-center gap-1">
                {isPinned ? (
                  <PinIcon
                    ...
                    className={`size-3 ${
                      isHighlighted
                        ? "text-foreground/60 dark:text-foreground/72"
...
const SidebarProjectItem = memo(function SidebarProjectItem(props: SidebarProjectItemProps) {
  ...
  const threadPinnedAtById = useUiStateStore((state) => state.threadPinnedAtById);
  ...
  const {
    projectPinnedThreads,
    projectRecentThreads,
    projectStatus,
    ...
    const projectPinnedThreads = visibleProjectThreads.filter(
      (thread) =>
        threadPinnedAtById[scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id))],
    );
    const projectRecentThreads = visibleProjectThreads.filter(
      (thread) =>
        !threadPinnedAtById[scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id))],
    );
    const orderedProjectThreads = [...projectPinnedThreads, ...projectRecentThreads];
    const projectStatus = resolveProjectStatusIndicator(
      visibleProjectThreads.map((thread) => resolveProjectThreadStatus(thread)),
    );
    return {
      orderedProjectThreadKeys: orderedProjectThreads.map((thread) =>
        scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id)),
      ),
      projectPinnedThreads,
      projectRecentThreads,
  ...
  }, [projectThreads, threadLastVisitedAts, threadPinnedAtById, threadSortOrder]);
  ...
  const {
    ...
    const hasOverflowingThreads = projectRecentThreads.length > sidebarThreadPreviewCount;
    const activeCollapsedThread =
      !projectExpanded && activeRouteThreadKey
        ? (projectRecentThreads.find(
            (thread) =>
              scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id)) ===
              activeRouteThreadKey,
          ) ?? null)
    ...
    const renderedThreads = projectExpanded
      ? [
          ...
          ...(isThreadListExpanded || !hasOverflowingThreads
            ? projectRecentThreads
            : projectRecentThreads.slice(0, sidebarThreadPreviewCount)),
        ]
      : [...projectPinnedThreads, ...(activeCollapsedThread ? [activeCollapsedThread] : [])];
    ...
    return {
      ...
      shouldShowThreadPanel: projectExpanded || renderedThreads.length > 0,
    };
  }, [
    activeRouteThreadKey,
    isThreadListExpanded,
          ...
          actionHandlers.set(id, () => {
            switch (action) {
              ...
              case "grouping":
                ...
                return;
              case "add-tag":
                openProjectTagDialog(member.physicalProjectKey, member.title);
                return;
              case "remove-tag":
                if (options?.tagId) {
  ...
  const handleThreadContextMenu = useCallback(
    async (threadRef: ScopedThreadRef, position: { x: number; y: number }) => {
      ...
      const isThreadPinned = threadPinnedAtById[threadKey] !== undefined;
      const explicitThreadTagIds = threadTagIdsByThreadKey[threadKey] ?? [];
      const projectTagIds = threadProject
        ? (projectTagIdsByProjectKey[threadProject.physicalProjectKey] ?? [])
        : [];
      const clicked = await api.contextMenu.show(
        [
          ...
          { id: "mark-unread", label: "Mark unread" },
          {
            ...
            icon: isThreadPinned ? "pin-off" : "pin",
          },
          { id: "add-tag", label: "Add Tag", icon: "tag" },
          ...explicitThreadTagIds.flatMap((tagId) => {
            const tag = tagById[tagId];
            return tag ? [{ id: `remove-tag:${tagId}`, label: `Remove tag: ${tag.name}` }] : [];
          }),
          ...projectTagIds.flatMap((tagId) => {
            const tag = tagById[tagId];
            return tag
              ? [
                  {
                    id: `remove-tag:${tagId}`,
                    label: `Remove inherited tag: ${tag.name}`,
                    disabled: true,
                  },
                ]
              : [];
    ...
    <>
      <div className="group/project-header relative">
        <SidebarMenuButton
          ...
          data-testid={`project-row-${project.projectKey}`}
        >
...
interface SidebarAllThreadsProjectMeta {
  label: string;
  tooltip: string;
  cwd: string | null;
}
...
function getCollapsedPinnedAndActiveThreads(
  ...
  const activeThread = recentThreads.find(
    (thread) =>
      scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id)) === activeRouteThreadKey,
  );
  return activeThread ? [...pinnedThreads, activeThread] : [...pinnedThreads];
}
...
const SidebarThreadSectionsList = memo(function SidebarThreadSectionsList(
  ...
  const handleThreadClick = useCallback(
    (
      event: React.MouseEvent,
      threadRef: ScopedThreadRef,
      orderedAllThreadKeys: readonly string[],
    ) => {
      const isMac = isMacPlatform(navigator.platform);
      const isModClick = isMac ? event.metaKey : event.ctrlKey;
      const isShiftClick = event.shiftKey;
      ...
      if (isShiftClick) {
        event.preventDefault();
        rangeSelectTo(threadKey, orderedAllThreadKeys);
        return;
      }
  ...
  const renderSection = (section: SidebarThreadListSection) => {
    ...
    const renderedSectionThreads = expanded ? section.threads : (section.collapsedThreads ?? []);
    const shouldRenderThreadList = expanded || renderedSectionThreads.length > 0;
    return (
      <div className="space-y-1" key={section.id} data-testid={`sidebar-section-${section.id}`}>
        ...
        {shouldRenderThreadList ? (
          <SidebarMenuSub
            ref={attachThreadListAutoAnimateRef}
            className="mx-0.5 my-0 w-full translate-x-0 gap-0.5 overflow-hidden px-1 py-0 sm:mx-1 sm:px-1.5"
          >
            {expanded && section.threads.length === 0 && section.emptyLabel ? (
              <SidebarMenuSubItem className="w-full" data-thread-selection-safe>
                <div
                  data-thread-selection-safe
                  className="flex h-6 w-full translate-x-0 items-center px-2 text-left text-[10px] text-muted-foreground/60"
            ...
            {renderedSectionThreads.map((thread) => {
              const threadKey = scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id));
              const projectMeta = projectMetaByThreadKey.get(threadKey);
              return (
                <SidebarThreadRow
      ...
      <SidebarGroup className="px-2 pb-0 pt-2">
        <div className="grid grid-cols-3 gap-0.5 rounded-md bg-muted/45 p-0.5">
          <Tooltip>
            <TooltipTrigger
              render={
                <button
                  ...
                  onClick={() => onSidebarViewModeChange("projects")}
                />
              }
            >
            ...
            <TooltipPopup side="right">Group threads by project</TooltipPopup>
          </Tooltip>
          <Tooltip>
            <TooltipTrigger
              render={
                <button
                  ...
                  onClick={() => onSidebarViewModeChange("all-threads")}
                />
              }
            >
...
export default function Sidebar() {
  ...
  const appSettingsConfirmThreadDelete = useClientSettings((s) => s.confirmThreadDelete);
  ...
  const projectMetaByThreadKey = useMemo(() => {
    return new Map(
      visibleThreads.map((thread) => {
        const threadKey = scopedThreadKey(scopeThreadRef(thread.environmentId, thread.id));
        const project = projectByScopedRef.get(
          scopedProjectKey(scopeProjectRef(thread.environmentId, thread.projectId)),
        );
        const label = project?.title ?? "Unknown project";
        const tooltip = project ? `${project.title}\n${project.workspaceRoot}` : "Unknown project";
        return [
          threadKey,
          {
            label,
  ...
  const { pinnedThreads, recentThreads } = useMemo(
    ...
    [threadPinnedAtById, visibleThreads],
  );
  const allSidebarThreadKeys = useMemo(
    () =>
  ...
  const projectVisibleSidebarThreadKeys = useMemo(
    () =>
