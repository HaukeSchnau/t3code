...
function ProjectDetail({
  ...
  const [savingProjectSetting, setSavingProjectSetting] = useState(false);
  const savingProjectSettingRef = useRef(false);
  const setBooleanOverride = async (
    key: "projectAutoPullOverrides",
    enabled: boolean | undefined,
  ) => {
    if (savingProjectSettingRef.current) return;
    savingProjectSettingRef.current = true;
    setSavingProjectSetting(true);
    try {
      ...
      savingProjectSettingRef.current = false;
      setSavingProjectSetting(false);
    }
  };
