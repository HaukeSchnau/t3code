...
function assignGraphPositions(
  nodes: readonly GitCommitGraphNodeContract[],
): Map<string, { x: number; y: number }> {
  const laneByChangeId = new Map<string, number>();
  let nextLane = 0;
  nodes.forEach((node) => {
    const lane = laneByChangeId.get(node.changeId) ?? nextLane++;
    laneByChangeId.set(node.changeId, lane);
    node.parentChangeIds.forEach((parentChangeId, parentIndex) => {
      if (!laneByChangeId.has(parentChangeId)) {
        laneByChangeId.set(parentChangeId, parentIndex === 0 ? lane : nextLane++);
      }
    });
  });
  ...
  nodes.forEach((node, index) => {
    positions.set(node.changeId, {
      x: (laneByChangeId.get(node.changeId) ?? 0) * 260,
      y: index * 112,
    });
  });
  return positions;
...
function buildFlowGraph(input: {
  ...
  const positions = assignGraphPositions(input.nodes);
  ...
  const flowEdges: Edge[] = input.nodes.flatMap((node) =>
    node.parentChangeIds
      ...
      .map((parentChangeId) => ({
        id: `${node.changeId}:${parentChangeId}`,
        source: node.changeId,
        target: parentChangeId,
        type: "smoothstep",
        className: "jj-commit-graph-edge",
      })),
  );
...
function JjCommitGraphInspector(props: {
  ...
  if (!selected) {
    return (
      <aside className="flex min-h-0 w-full shrink-0 flex-col border-t border-border bg-background p-4 text-muted-foreground text-sm lg:w-80 lg:border-l lg:border-t-0">
  ...
  return (
    <aside className="flex min-h-0 w-full shrink-0 flex-col border-t border-border bg-background lg:w-88 lg:border-l lg:border-t-0">
