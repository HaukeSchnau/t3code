...
describe("streaming row projection", () => {
  ...
  it("reuses long-thread rows through detail events, selectors, and attachment previews", () => {
    ...
    const checkpointSummaryByAssistantMessageId = new Map(
      history.map(({ checkpoint }) => [checkpoint.assistantMessageId!, checkpoint] as const),
    );
    const revertTurnCountByUserMessageId = new Map();
    const project = () => {
      ...
      projection = deriveMessagesTimelineRowsWithState(
        {
          ...
          turnDiffSummaryByAssistantMessageId: checkpointSummaryByAssistantMessageId,
...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("projects assistant diff summaries and user revert counts onto the affected rows", () => {
    ...
    const rows = deriveMessagesTimelineRows({
      ...
      activeTurnStartedAt: null,
      turnDiffSummaryByAssistantMessageId: new Map([
        [assistantTurnDiffSummary.assistantMessageId, assistantTurnDiffSummary],
      ]),
  ...
  it("marks user rows editable independently from checkpoint revert metadata", () => {
    const rows = deriveMessagesTimelineRows({
      timelineEntries: [
        {
          id: "user-entry",
          ...
          message: {
            id: "user-1" as never,
            role: "user",
            text: "I should be able to clean this up",
            turnId: null,
            createdAt: "2026-01-01T00:00:00Z",
            updatedAt: "2026-01-01T00:00:00Z",
            streaming: false,
...
describe("computeStableMessagesTimelineRows", () => {
  it("replaces a cached work toggle when its icon presentation changes", () => {
    const initialRow: Extract<MessagesTimelineRow, { kind: "work-toggle" }> = {
      kind: "work-toggle",
      id: "work-toggle:1",
      createdAt: "2026-01-01T00:00:00Z",
    ...
    const enrichedRow: Extract<MessagesTimelineRow, { kind: "work-toggle" }> = {
      ...initialRow,
