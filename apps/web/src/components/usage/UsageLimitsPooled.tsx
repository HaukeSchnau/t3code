...
function AccountAvatar({
  ...
  if (account.redeem && "instanceId" in account.redeem.input) {
    return (
      <ProviderInstanceIcon
        instanceId={account.redeem.input.instanceId}
...
function PoolSection({ pool, now }: { readonly pool: LimitPool; readonly now: number }) {
  ...
  const driver = getDriverOption(pool.driver);
  const label = driver?.label ?? String(pool.driver);
  const Icon = driver?.icon;
  return (
    <section className="flex flex-col gap-3">
      <h2 className="flex items-center gap-2 text-sm font-medium text-foreground">
        {Icon ? <Icon className="size-4 text-foreground/80" aria-hidden /> : null}
        {label}
      </h2>
