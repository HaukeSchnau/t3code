...
/** Desktop fallback when the probe fails or finds nothing. */
...
export function useRemoteCapableEditors(): ReadonlyArray<EditorId> {
  ...
  const [editors, setEditors] = useState<ReadonlyArray<EditorId>>(
    () =>
      cachedProbedEditors ??
      (probe === undefined ? BROWSER_REMOTE_EDITORS : REMOTE_FALLBACK_EDITORS),
  );

  useEffect(() => {
    if (cachedProbedEditors !== null || probe === undefined) {
      return;
    }
    let cancelled = false;
    probe().then(
