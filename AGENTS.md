...
- `node scripts/setup-worktree.ts` installs with pinned pnpm, or reuses the native devenv dependency task. Vite warms its browser dependency cache when the dev server starts. Worktrees get this from the t3.json setup script; if module resolution looks broken, it probably did not run.
