...
| Skill pack          | A named bundle of skills from the environment catalog. Packs add to provider-native and project skills; core skills are always active.    |
| Skill scope         | The materialized pack selection for one thread. It can be pending while the provider applies it or degraded when some skills cannot load. |
