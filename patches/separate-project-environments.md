...
Independent jj environments are an opt-in infra launcher mode. The registry's optional `workspace.visibleRoot` gives the agent-visible path; T3 retains the canonical host root as the project/session identity. Namespace-relative paths use the resolved host cwd, including when a checkout or its parent is a symlink. Codex thread startup and Claude SDK options use the visible cwd, while subprocess launch uses the host cwd. Local MCP endpoints use the environment's host gateway. This is intentionally coupled to the private runtime rather than a generic container abstraction.
