...
Codex, Claude, and OpenCode receive a short notice that `t3 thread` exists and that
`--help` lists its commands. The notice does not prescribe delegation or worker workflows.
Cursor, Grok, and Antigravity retain their native prompt behavior.
