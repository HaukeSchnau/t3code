...
- Every current adapter declares `assistantTranscriptRecovery: "none"`. On this path every parent assistant delta is durably dispatched by its own deterministic projection command, and every subagent transcript change durably publishes a `subagent.thread` activity, before the exact journal row is retired. Transient persistence failures retry with bounded spacing instead of being logged and dropped.
...
- Unary replay plus shell and thread catch-up share one low-overhead observation lifecycle. Metrics and structured completion logs expose the persisted tail, pages/batches, events scanned before filtering, replay items emitted, duration, live-buffer high-water mark, and buffered overlap that clients dedupe. Metric labels contain only the bounded flow and outcome dimensions; project, thread, command, and event identifiers are deliberately excluded. Failure and interruption finalize the same observation so stalled or cancelled train-network reconnects remain measurable.
  Finalization performs only a fixed number of in-memory metric/counter updates on the handoff path; structured log publication is forked so a slow log sink cannot delay draining the bounded live buffer.
...
- Full subagent activity snapshots and parent assistant delta commands are published for each current-provider semantic transcript change. This is intentionally proportional to real assistant transcript delta count; a future specialized durable subagent delta event could reduce cumulative snapshot payload amplification if assistant streams—not command output—become a measured bottleneck.
