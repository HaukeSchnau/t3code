...
- Buffered assistant text exists only for journal-backed events. Approval, user-input, item, turn, runtime-error, and session-exit boundaries finalize it with message-specific idempotency keys.

### Projections and reducers

- The projection pipeline routes each event only to projectors that handle it, then advances all projector cursors in one atomic transaction. Mixed-cursor bootstrap retains its replay semantics. The turn projector must receive compaction and turn-start failure activities so completed or canceled requests clear their pending turn rows.
