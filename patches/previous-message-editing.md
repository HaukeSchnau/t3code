...
- Rollback must persist the provider's post-rollback resume cursor before the next recovered turn. For Codex, `thread/rollback` can return a new provider thread id, so `CodexSessionRuntime` updates `resumeCursor` from the rollback response and `ProviderService.rollbackConversation` persists the active session state after rollback.
